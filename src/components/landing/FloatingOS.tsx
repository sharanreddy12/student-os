...
import {
  Calendar, FileText, CheckCircle2, Users, Sparkles, BarChart3,
} from "lucide-react";
  ...
  },
  {
    ...
    body: (
      <div className="space-y-2 text-xs">
        ...
        <div className="rounded-lg px-3 py-2 text-foreground/90"
...
function Row({ time, label, active }: { time: string; label: string; active?: boolean }) {
  return (
    <div className={cn("flex items-center gap-2 rounded-md px-2 py-1.5",
      ...
      {active && <span className="ml-auto h-1.5 w-1.5 rounded-full bg-[color:var(--neon-cyan)] shadow-[0_0_8px_var(--neon-cyan)]" />}
    </div>
  );
}
...
function Ring({ value }: { value: number }) {
  ...
  return (
    <svg width="60" height="60" viewBox="0 0 60 60" className="-rotate-90">
      <circle cx="30" cy="30" r="22" stroke="white" strokeOpacity="0.1" strokeWidth="5" fill="none" />
      <circle cx="30" cy="30" r="22" stroke="url(#g)" strokeWidth="5" fill="none"
      ...
      <div className="animate-float" style={{ animationDelay: `${idx * 0.6}s` }}>
        <div className="glass-strong p-4 shadow-elevate">
          <div className="mb-3 flex items-center gap-2">
            ...
            <span className="text-xs font-medium tracking-wide text-foreground/90">{panel.title}</span>
          </div>
          {panel.body}
        </div>
      </div>
